...
ROOT_DIR = pathlib.Path(**file**).parent.parent
RULES_DIR = ROOT_DIR / “detection-rules”
RULE_CONFIG_FILE = ROOT_DIR / “rule_config.yaml”

# Maps a rule-name prefix (lowercase) to the folder name under detection-rules/.

# To add a new technology, simply add a new entry here.

# Example: a rule named “cortex_suspicious_login.yaral” will be placed in

# detection-rules/Cortex/

TECHNOLOGY_PREFIX_MAP: dict[str, str] = {
“gcp”: “GCP”,
“okta”: “Okta”,
“cortex”: “Cortex”,
# Add more technologies here, e.g.:
# “aws”: “AWS”,
# “azure”: “Azure”,
...
```
The prefix is determined by splitting the rule name on the first underscore.
For example:
  - "gcp_suspicious_activity" -> detection-rules/GCP/
  - "okta_brute_force"        -> detection-rules/Okta/
  - "cortex_lateral_movement" -> detection-rules/Cortex/
  - "unknown_rule"            -> detection-rules/Other/

To support a new technology, add an entry to TECHNOLOGY_PREFIX_MAP at the
top of this file.

Args:
  rule_name: The name of the rule (without file extension).
  rules_dir: The root detection-rules directory.

Returns:
  A Path to the technology subfolder, which is created if it doesn't exist.
"""
prefix = rule_name.split("_")[0].lower()
folder_name = TECHNOLOGY_PREFIX_MAP.get(prefix, "Other")
tech_dir = rules_dir / folder_name
...
```
# Use rglob to find .yaral files across all technology subfolders
rule_files = list(rules_dir.rglob("*.yaral"))
non_rule_files = [
    file_path
    for file_path in rules_dir.rglob("*")
    if file_path.is_file() and not file_path.name.endswith(".yaral")
]
...
“”“Dump rules to local files organised into technology subfolders.

```
Each rule is placed in a subfolder under rules_dir based on the technology
prefix in its name (e.g. gcp_ -> GCP/, okta_ -> Okta/). Unknown prefixes
fall back to an Other/ subfolder. The mapping is defined in
TECHNOLOGY_PREFIX_MAP at the top of this module.
"""
LOGGER.info("Writing %s rule files to %s", len(self.rules), rules_dir)
for rule in self.rules:
  tech_dir = Rules.get_technology_folder(rule.name, rules_dir)
  rule_file_path = tech_dir / f"{rule.name}.yaral"
  LOGGER.debug("Writing rule %s to %s", rule.name, rule_file_path)
